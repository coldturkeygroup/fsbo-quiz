...
class FrontDesk
{
    ...
    {
        $this->api_key = get_option('pf_fsbo_quiz_frontdesk_key');
            ...
            if ($this->api_key != null || $this->api_key != '') {
                $response = $this->guzzle->post($this->api_base . 'campaigns/', [
                    'body' => [
                        'key' => $this->api_key,
                        'title' => $title,
                        'description' => 'Campaign for Platform FSBO Quiz',
                        'type' => 'Platform',
                        'total_cost' => '10000',
                        'source' => $permalink
                    ]
    ...
    public function add_success_var($location)
    {
        ...
        return esc_url_raw(add_query_arg(['pf_fsbo_quiz_frontdesk_success' => true], $location));
    }
    ...
    public function add_error_var($location)
    {
        ...
        return esc_url_raw(add_query_arg(['pf_fsbo_quiz_frontdesk_error' => true], $location));
    }
    ...
    {
        if (isset($_GET['pf_fsbo_quiz_frontdesk_error']))
            echo '<div class="error">
	      			<p>A Campaign with this URL already exists. No new Platform CRM Campaign has been created.</p>
                  </div>';

        if (isset($_GET['pf_fsbo_quiz_frontdesk_success']))
            echo '<div class="updated">
	      			<p>A Campaign for this FSBO Quiz has been successfully setup on Platform CRM!</p>
                  </div>';
    }

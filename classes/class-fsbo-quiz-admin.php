...
\namespace ColdTurkey\FSBOQuiz;

if (!defined('ABSPATH')) exit; // Exit if accessed directly.

class FSBOQuiz_Admin
{
    private $dir;
    private $file;
    private $assets_dir;
    ...
    /**
     * Basic constructor for the FSBO Quiz Admin class
     *
     * @param string $file
     */
    ...
    public function add_menu_item()
    {
        ...
        add_submenu_page('edit.php?post_type=' . $this->token, 'FSBO Quiz Settings', 'Settings', 'manage_options', $this->token . '_settings', [
            $this,
            'settings_page'
        ]);
    }
    ...
    public function admin_notices()
    {
        ...
        if ($wp_version < 3.5) {
            ?>
            <div class="error">
                <p><?php printf(__('%1$sFSBO Quiz%2$s requires WordPress 3.5 or above in order to function correctly. You are running v%3$s - please update now.', $this->token), '<strong>', '</strong>', $wp_version); ?></p>
            </div>
            <?php
        }
    }
    ...
    public function register_settings()
    {
        ...
        // Add settings fields
        add_settings_field($this->token . '_slug', __('URL slug for FSBO Quiz pages:', $this->token), [
            $this,
            'slug_field'
    ...
    {
        echo '<p>' . __('These are a few simple settings for setting up your FSBO Quiz pages.', $this->token) . '</p>';
    }

    /**
     * Create the slug field for the Settings page.
     * The slug field allows users to choose which
     * subdirectory their FSBO Quiz pages are nested in.
     *
     */
    public function slug_field()
    {
        ...
        echo '<input id="slug" type="text" name="' . $this->token . '_slug" value="' . $data . '"/>
				<label for="slug"><span class="description">' . sprintf(__('Provide a custom URL slug for the FSBO Quiz pages.', $this->token)) . '</span></label>';
    }
    ...
    /**
     * Define the basic FSBO Quiz URL
     *
     */
    public function pf_fsbo_quiz_url()
    {
        $pf_fsbo_quiz_url = $this->home_url;

        $slug = get_option($this->token . '_slug');
        if ($slug && strlen($slug) > 0 && $slug != '') {
            $pf_fsbo_quiz_url .= $slug;
        } else {
            $pf_fsbo_quiz_url .= '?post_type=' . $this->token;
        }

        echo '<a href="' . esc_url($pf_fsbo_quiz_url) . '" target="_blank">' . $pf_fsbo_quiz_url . '</a>';
    }
